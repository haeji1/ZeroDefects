# bokeh
from bokeh.models import ColumnDataSource, DatetimeTickFormatter, BasicTickFormatter, DatetimeTicker,SingleIntervalTicker
...
#
#     # 데이터 프레임 값 계산하는 함수 호출
#     calc_df_values(df_list)
#
...
#
#             df["Time"] = (pd.to_datetime(df["Time"].str.replace("Z", ""), utc=True) - start_time)
#
#             # 색상 선택
...
def draw_dataframe_to_graph(df_list, facility_list):
    ...
    if len(df_list) == 1:  # 데이터프레임이 하나일 때
        ...
        for i, df in enumerate(df_list):
            ...
            # 각 컬럼 데이터를 그래프에 추가하면서 다른 색상 지정
            for j, column_name in enumerate(df.columns[1:]):
                p.line(x='Time', y=column_name, source=source, legend_label=f"{df_name} - {column_name}",
                       color=color)
...
# 사이클에 대한 max, min, 평균 값 구하기
def calc_df_values(df_list):
