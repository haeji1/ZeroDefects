...
export const getTGLife = async (tGLifeInfo: any) => {
  try {
    console.log("api ", tGLifeInfo);
    const res = await axios.post("http://localhost:8000/facility/read/tg", tGLifeInfo);
    console.log("배치 데이터 가져오기 완료.");
    console.log(res);
    return res;
  } catch (err) {
    console.log("fail to get TGLife data", err);
  }
};
